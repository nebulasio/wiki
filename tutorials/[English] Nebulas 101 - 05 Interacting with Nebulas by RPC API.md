...
make
```
Generate the corresponding go version of the grpc interface code. Go version of the official code has been generated, the use of time can not be re-generated. The gRPC port can be modified in the configuration file(eg: `conf/default/config.conf`). Configuration items in the port configuration items are as follows:
