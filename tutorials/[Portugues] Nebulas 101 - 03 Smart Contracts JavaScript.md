...
Através deste tutorial irá aprender como escrever, implantar, e executar smart contracts em Nebulas.
...
2. O código do smart contract tem de ter um método init(), que apenas será executado uma vez durante a implantação;
...
## Implantação de smart contracts

Aprendeu a escrever um smart contract em Nebulas, e agora precisamos de o implantar na chain.
Antes, foi introduzido como fazer uma transação em Nebulas, e usámos a interface sendTransaction() para iniciar a transferência. Implantar um smart contract em Nebulas é efectuado ao enviar uma transação, chamando a interface sendTransaction(), apenas com parâmetros diferentes. 
...
Obedecemos a uma convenção que se `from` e `to` tiverem o mesmo endereço, `contract` não é nulo e `binary` é nulo, assumimos que estamos a implantar um smart contract.

- `from`: endereço do criador
- `to`: endereço do criador 
- `value`: deve ser `"0"` ao implantar o contracto;
...
- `gasPrice`: o gasPrice usado para implantar o smart contract, que pode ser obtido via [`GetGasPrice`](https://github.com/nebulasio/wiki/blob/master/rpc.md#getgasprice), or using default values: `"1000000"`;
- `gasLimit`: o gasLimit para implantar o contracto. Pode obter uma estimativa do consumo do gás para implantação via [`EstimateGas`](https://github.com/nebulasio/wiki/blob/master/rpc.md#estimateGas), e não pode usar o valor padrão. Também pode definir um valor maior. O consumo real de gás é decidido pela execução da implantação.
- `contract`: a informação do contracto, os parâmetros passados na implantação do contracto
...
Exemplo de implantação de um smart contract usando curl:
...
O valor retornado para implantar um smart contract é o endereço hash da transação `txhash` e o endereço da implantação do contracto é `contract_address`.
A obtenção deste valor não garante o sucesso da implantação do contracto, porque sendTransaction() é um processo assíncrono, que precisa de ser empacotado pelo mineiro. Tal como a transação prévia, a transferência não chega em tempo real, e depende da velocidade da empacotação do mineiro. Portanto precisa de esperar cerca de 1 minuto, e depois pode verificar se o contracto foi implantado com sucesso, ao inquirir o endereço do contracto, ou chamando este smart contract.

> **Verifique se a implantação do contracto foi bem sucedida**
>
> Verifique o recibo da transação da implantação via [`GetTransactionReceipt`](https://github.com/nebulasio/wiki/blob/master/rpc.md#gettransactionreceipt) para verificar se o contracto foi implantado com sucesso.
> ```bash
...
> ```
> Como mostrado acima, o estado da transação da implantação torna-se 1. Significa que o contracto foi implantado com sucesso.
...
> ```
> Como visto acima, o estado da transação da implantação torna-se 1. Significa que o método do contracto foi executado com sucesso.
